...
class TestDomainGateway(unittest.TestCase):
    ...
    def test_pause_persists_state(self):
        """测试暂停操作会自动持久化状态。"""
        g = _make_gateway(paused=False)
        g.pause(restart_after_idle=False, is_currently_idle=False)
        g._state_repo.set_paused.assert_called_once_with(True)
    ...
    def test_resume_persists_state(self):
        ...
        g.resume()
        g._state_repo.set_paused.assert_called_once_with(False)
