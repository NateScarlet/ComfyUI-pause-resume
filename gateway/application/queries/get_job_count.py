...
from gateway.shared.models import TaskFilters
...
class GetJobCountQueryHandler:
    ...
    async def handle(self, filter_by: TaskFilters) -> int:
        """从网关拦截的任务队列中查询符合条件的总任务数。"""
        return self._queue_reader.get_task_count(filter_by=filter_by)
